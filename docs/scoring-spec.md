...
| Tier 2 | Florida DNC / FTSA | 13 | live when API-verified (Screen 04); else 0, flagged |
| Tier 3 | Quiet hours | 9 | live (Screen 03) |
| Tier 4 | National DNC registry | 7 | live when API-verified (Screen 04); else 0, flagged |

**Stacking add-on** (over *distinct verified* theories present): **+3** for a 2nd, **+6** for 3+.
**Caps at 24.**

- Tiers 2 and 4 score only when the registration was confirmed by the automated
  RealValidation DNC lookup of the client's number (see screening-spec Screen 04); a confirmed
  FL and national registration are distinct theories for stacking. Unconfirmed, they contribute
  neither base nor stacking — only a flagged unknown ("DNC status unverified — could raise score").
...
| Input | MVP | Future |
|---|---|---|
| Claim Tiers 2 & 4 (DNC) | scored on API-verified registration (RealValidation DNC lookup) | — |

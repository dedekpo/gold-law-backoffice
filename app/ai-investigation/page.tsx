...
import { NewCaseModal, RerunDialog } from "@/components/new-case-modal";
import { OpportunityPreviewPane } from "@/components/opportunity-preview";

/**
 * The AI desk: runs the automated investigation (screening, DNC, company
 * identification) over an opportunity's evidence. The default flow drains the
 * '👀 Ready for AI Investigation' stage from the sidebar docket; a pasted GHL
 * URL or a manual file upload enters through the optional "New case" door.
 */
...
export default function AiInvestigationPage() {
  ...
  // ------------------------------------------------------------------
  // The Ready-for-AI docket: open opportunities in the stage, the default
  // way a run starts. Clicking a row opens the opportunity's preview (its
  // evidence + any previous run); the run starts from the preview's button —
  // importing the evidence, same as pasting the opportunity's URL.
  ...
  const [preview, setPreview] = useState<{
    id: string;
    data: OpportunityPreviewResponse | null;
    error: string | null;
  } | null>(null);
  const [importingId, setImportingId] = useState<string | null>(null);
  const [actionError, setActionError] = useState<string | null>(null);
  // Import held for confirmation because the agent already ran.
  const [pendingRerun, setPendingRerun] = useState<ImportResponse | null>(null);
  const [rerunBusy, setRerunBusy] = useState(false);

  useEffect(() => {
    void (async () => {
      try {
        const res = await fetch("/api/investigation/queue?desk=ai");
  ...
  // Load the read-only preview of the selected docket entry (no DNC lookup,
  // no writes — those happen when the run starts).
  useEffect(() => {
    ...
    void (async () => {
      try {
        const res = await fetch(
          `/api/opportunity/preview?opp=${encodeURIComponent(selectedQueueId)}`,
        );
        const data = (await res.json().catch(() => null)) as
          | (OpportunityPreviewResponse & { error?: string })
          | null;
        if (!res.ok || !data) {
          throw new Error(data?.error ?? `Loading failed: ${res.status}`);
        }
        if (!cancelled) {
          setPreview({ id: selectedQueueId, data, error: null });
        }
      } catch (err) {
        if (!cancelled) {
          setPreview({
  ...
  function selectQueueEntry(entry: QueueEntry) {
    setSelectedCaseId(null);
    setActionError(null);
    setSelectedQueueId(entry.id);
  }

  async function launchImport(data: ImportResponse) {
    const inputs = await downloadImportFiles(data);
    await createCase(inputs, importMeta(data));
    // A started run leaves the Ready-for-AI stage (the import moved it to
    // 'AI Under Investigation'); it now lives in "This session".
    setQueue((prev) =>
      prev ? prev.filter((e) => e.id !== data.opportunity.id) : prev,
    );
    setSelectedQueueId(null);
  }

  async function startQueueEntry(entry: QueueEntry) {
    setActionError(null);
    setImportingId(entry.id);
    try {
      const data = await importOpportunity({ opportunityId: entry.id });
      if (data.files.length === 0) {
        throw new Error(
          `"${entry.name}" has no files in Violation Screenshots or Violation Audio Files.`,
        );
      }
      if (data.existingRun) {
        // The agent already ran for this opportunity — ask before re-running.
        setPendingRerun(data);
        return;
      }
      await launchImport(data);
    } catch (err) {
      setActionError(
        err instanceof Error ? err.message : "Could not start the case.",
      );
    } finally {
      setImportingId(null);
    }
  ...
  async function confirmRerun() {
    ...
    setRerunBusy(true);
    try {
      await launchImport(pendingRerun);
      setPendingRerun(null);
    } catch (err) {
      // Keep the dialog open so the error is read in context.
      setActionError(
        err instanceof Error ? err.message : "Could not start the case.",
      );
    } finally {
  ...
  const selectedQueueEntry =
    (!selectedCase && queue?.find((e) => e.id === selectedQueueId)) || null;

  return (
    <main className="flex min-h-0 flex-1 bg-paper">
      <CaseSidebar
        cases={cases}
      ...
      <div className="flex min-h-0 flex-1 bg-card">
        {selectedCase ? (
          ...
          <OpportunityPreviewPane
            entry={selectedQueueEntry}
            preview={
              preview && preview.id === selectedQueueEntry.id
                ? { data: preview.data, error: preview.error }
                : null
            }
            importing={importingId === selectedQueueEntry.id}
            actionError={pendingRerun ? null : actionError}
            onRun={() => void startQueueEntry(selectedQueueEntry)}
          />
        ) : (
          <div className="flex flex-1 items-center justify-center p-8">
            <div className="flex max-w-md gap-4">
              <div
                aria-hidden
                className="w-[5px] shrink-0 border-x border-stamp/40"
              />
              <div className="flex min-w-0 flex-col gap-3 font-sans">
                ...
                <p className="text-sm leading-6 text-soft">
                  Pick an opportunity from the docket to review its evidence
                  and start the run. The agent evaluates the evidence against
                  the TCPA rubric, checks the client&rsquo;s number against
                  the DNC registries, and investigates the company behind the
                  violation — then files the findings back onto the
                  opportunity.
                </p>
                <p className="text-sm leading-6 text-soft">
                  &ldquo;New case&rdquo; takes a pasted GHL URL, or a manual
                  upload for evidence that isn&rsquo;t on an opportunity yet.
                </p>
              </div>
            </div>
          </div>
      ...
      {pendingRerun && (
        <RerunDialog
          status={pendingRerun.existingRun?.status ?? null}
          busy={rerunBusy}
          error={actionError}
          onCancel={() => {
            setPendingRerun(null);
